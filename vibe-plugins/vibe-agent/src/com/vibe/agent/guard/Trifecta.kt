 ...
 * одной сессии: агент прочитал приватное, в его контекст попало чужое указание, и у него есть канал
 * наружу. Отдельно каждое из трёх — работа, вместе — способ вынести данные.
 *
 * Сессия, а не ход (13.09.2026): утечка, растянутая на несколько ходов — секрет в первом, чужой текст
 * в третьем, `curl` в пятом, — ни в одном ходе всех трёх признаков не имела и не ловилась вовсе.
 * Порог прежний, поэтому предупреждение по-прежнему редкое. Шаг пайплайна считает своё отдельно.
 *
 ...
 * и предупреждение, которое видели двадцать раз зря, перестают читать. Три признака пересекаются
 * редко даже за сессию, поэтому предупреждение остаётся редким.
 *
...
object Trifecta {
  ...
  /** Все три признака за одну сессию — повод спросить человека. */
