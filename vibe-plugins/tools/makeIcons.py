...
ICONS = {
    ...
    "vibeServer": (SERVER,
        '<rect x="2.5" y="2.5" width="11" height="4.4" rx="1.2" stroke="{C}" stroke-width="1"/>'
        '<rect x="2.5" y="9.1" width="11" height="4.4" rx="1.2" stroke="{C}" stroke-width="1"/>'
        '<circle cx="5" cy="4.7" r="0.8" fill="{C}"/><circle cx="5" cy="11.3" r="0.8" fill="{C}"/>',
        '<rect x="3" y="3" width="14" height="5.6" rx="1.6" stroke="{C}" stroke-width="1.4"/>'
        '<rect x="3" y="11.4" width="14" height="5.6" rx="1.6" stroke="{C}" stroke-width="1.4"/>'
        '<circle cx="6.2" cy="5.8" r="1" fill="{C}"/><circle cx="6.2" cy="14.2" r="1" fill="{C}"/>',
    ),
}
...
def files():
    for name, (out, body16, body20) in ICONS.items():
        yield out, f"{name}.svg", svg(16, body16, LIGHT)
        yield out, f"{name}_dark.svg", svg(16, body16, DARK)
        yield out, f"{name}@20x20.svg", svg(20, body20, LIGHT)
        yield out, f"{name}@20x20_dark.svg", svg(20, body20, DARK)


def main() -> int:
    check = "--check" in sys.argv
    problems = []
    for out, filename, content in files():
    ...
    print(f"  записано {len(ICONS) * 4} файлов ({len(ICONS)} значков)")
    return 0
