...
# И в обратную сторону. Бандл, собранный скриптом, но не объявленный провайдером, платформе не
# достаётся вовсе: грамматика едет мёртвым грузом, а язык остаётся без подсветки — и никто об этом
# не узнает, потому что файл на месте и проверка выше зелёная.
for name in $SHIPPED; do
  case " $DECLARED " in
    *" $name "*) : ;;
    *) say "✖ бандл «$name» едет в дистрибутиве, но провайдер его не объявляет — подсветки не будет"; fail=1 ;;
  esac
done
