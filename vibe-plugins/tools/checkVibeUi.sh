...
while IFS= read -r page; do
  ...
  # Своего скролла мало — платформу надо ПОПРОСИТЬ не заворачивать страницу в свой. Без
  # Configurable.NoScroll ConfigurableCardPanel кладёт нашу страницу-со-скроллом во второй скролл и
  # выдаёт ей всю предпочтительную ширину: наш скролл тогда не решает ничего, и содержимое уезжает за
  # край диалога. Урок был записан 24.08.2026 и стоял комментарием на ОДНОЙ странице из десяти — ровно
  # на тех трёх, где он был, владелец обрезки не видел никогда; на семи остальных видел четырежды.
  if grep -q 'SettingsUi.page(' "$page" && ! grep -q 'Configurable.NoScroll' "$page"; then
    echo "ОШИБКА: страница со своим скроллом не объявляет Configurable.NoScroll — платформа завернёт её во второй:"
    echo "    $page"
    echo "  Добавьте к объявлению класса: Configurable, Configurable.NoScroll"
    status=1
  fi
