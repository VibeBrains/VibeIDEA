...
# Образ обязан быть ПОДПИСАН до штампа, а не после.
#
# Штамп фиксирует sha256 файлов; подпись меняет содержимое образа, поэтому подписанное после штампа
# перестанет совпадать с заштампованным — опубликуем одно, а проверено другое. Плюс смысл самой
# подписи: связка ключей macOS выдаёт доступ к сохранённым ключам провайдеров ПО ПОДПИСИ, и
# неподписанная сборка каждый раз выглядит новым приложением (за день владелец вводил пароль
# по три-четыре раза, 18.09.2026).
#
# Проверяется фактом — `codesign` по приложению внутри образа, а не наличием шага в мануале.
case "$(uname -s)" in
  Darwin)
    SIGN_MNT=$(hdiutil attach -nobrowse -readonly "$IMAGE" 2>/dev/null | grep -o '/Volumes/.*$' | tail -1 || true)
    if [ -z "$SIGN_MNT" ]; then
      echo "✖ образ $IMAGE не монтируется — подпись проверить нечем"
      exit 1
    fi
    SIGN_AUTHORITY=$(codesign -dv --verbose=2 "$SIGN_MNT"/*.app 2>&1 | sed -n 's/^Authority=//p' | head -1 || true)
    hdiutil detach "$SIGN_MNT" -force >/dev/null 2>&1 || true
    if [ -z "$SIGN_AUTHORITY" ]; then
      echo "✖ приложение в образе НЕ подписано — штамп зафиксировал бы хеши, которые подпись потом изменит"
      echo "  Подпишите до штампа: ./vibe-plugins/tools/macSelfSign.sh sign $IMAGE"
      exit 1
    fi
    echo "  подпись образа: $SIGN_AUTHORITY"
    ;;
esac
