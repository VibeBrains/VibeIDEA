...
**Удобный путь: Settings → Tools → VibeIDEA → Провайдеры** — поле ключа на каждого провайдера, значение сохраняется в защищённое хранилище ОС (Keychain) под `apiKeyRef` (или id провайдера, если `apiKeyRef` не задан).
...
- `vision` — тристейт: `true`/`false`/отсутствует (неизвестно). При `extends` и слиянии global/workspace незаданный `vision` наследуется от базовой записи той же модели.
