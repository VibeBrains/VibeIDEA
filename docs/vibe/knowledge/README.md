...
| [phpOnWindows.md](languages/phpOnWindows.md) | [грабли+решение] Встроенный Phpactor на Windows не стартует никогда: его phar требует `ext-posix`, которого в Windows-сборках PHP не существует. Движок PHP стал выбором (`PhpEngine`): АВТО = Phpactor на macOS/Linux, Intelephense на Windows; один вход `vibePhp` в LSP4IJ, иначе два сервера на `*.php` запустятся оба. Плюс побочная находка: гейт локализации считал упоминание ключа в комментарии использованием. |
