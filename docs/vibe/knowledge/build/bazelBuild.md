...
- `.icns` собирает `vibe-plugins/tools/makeAppIcon.sh`: `rsvg-convert` рисует каждый размер iconset сразу в его пикселях (до 32 px — упрощённый `vibeidea_16.svg`), потом `iconutil -c icns`. Прежний ручной рецепт через `qlmanage -t` рисовал SVG размером 16 в угол большого холста: размеры 16, 16@2x и 32 месяц уходили с логотипом в углу пустого поля, а список Finder и `.ico` Windows (он берёт мелкие размеры из того же `.icns`) показывали пятно вместо значка (25.09.2026). В Dock и «О программе» этого не видно — поэтому рядом с иконкой лежит `vibeidea.icns.sources` (хэши обоих SVG и самой иконки), и `checkVibeUi.sh` падает, если иконка собрана не скриптом или логотип правили без пересборки.
