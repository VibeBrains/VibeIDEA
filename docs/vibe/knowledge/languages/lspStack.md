...
## Почему `Ext6` подчёркивается, а в PhpStorm нет (04.09.2026)

PhpStorm понимает JavaScript **своим закрытым движком**: он индексирует все `.js` проекта, и
присваивание `Ext6 = …` считает объявлением — отсюда «Choose Declaration» с полутора десятками
кандидатов. В intellij-community движка JS нет вовсе (проверено по дереву: `jshell` — это Java,
`jsonpath` — JSON), поэтому язык у нас держит `vtsls`.

tsserver работает не «со всеми файлами проекта», а с **программой**. Без `jsconfig.json` он для
каждого открытого файла собирает «выведенный проект»: сам файл плюс достижимое через
`import`/`require`/`/// <reference>`. ExtJS грузит классы своим загрузчиком в рантайме — импортов
нет, и файлы с объявлениями в программу не попадают.

Отсюда правило: **на большой JS-базе без модулей `jsconfig.json` обязателен**, и это первое, что
надо предлагать, а не `.d.ts` (он нужен лишь для глобалов, заводимых способом, который TypeScript
объявлением не считает, — `window.Ext6 = …`). Подсказку теперь даёт сама IDE при открытии `.js`.

Честная граница: даже с `jsconfig.json` разрешение будет хуже штормовского — типы выводятся из
присваиваний, а не из знания библиотеки, и на большой базе сервер будет заметно думать. Сравняться
здесь нельзя, пока нет открытого движка JS.
