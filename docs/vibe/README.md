...
- [manuals/mcpServersSpec.md](manuals/mcpServersSpec.md) — свои MCP-серверы человека в `.vibe/mcp.json`:
    форма файла, поля, права чужих инструментов и что говорится на ошибку
...
- [manuals/rolesSpec.md](manuals/rolesSpec.md) — спека `.vibe/roles.json`: куда пишет роль `qa` по умолчанию, общая с VibeIDE (самодостаточная, для LLM).
- [manuals/directChatTools.md](manuals/directChatTools.md) — как включить инструменты в прямом чате с моделью: общая память VibeMemory, чтение без вопросов, запись с согласия.
...
- [manuals/patrolsSpec.md](manuals/patrolsSpec.md) — спека дежурных проверок `.vibe/patrols.json`: дешёвая проба по расписанию решает, звать ли модель (самодостаточная, для LLM).
