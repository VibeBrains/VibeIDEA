...
**Ответы без воды.** Сжатые ответы включены по умолчанию: модель отвечает без вежливостей, оговорок и пересказа
своих шагов, а термины, код и тексты ошибок оставляет дословно. Уровень меняется кнопкой быстрых настроек в ряду
композера, там же глубина рассуждений, офлайн-режим и минимализм.
...
**Дизайн-детектор.** 85 правил по живой странице в двух вьюпортах: пол качества (контраст, зоны
...
**Детектор нейрослопа.** Тексты, которые пишет агент — README, документация, заметки, тексты страницы, —
проверяются общим каталогом примет машинного письма на русском и английском: штампы, пустые обороты,
риторические заготовки, ритм. Находка приходит со строкой и способом исправить, гейт хода может вернуть
агента к тексту, а навык `anti-slop` добавляет рецензента, который ловит то, чего правила не видят, —
выдуманные факты.
