...
### platform/platform-api/…/LafManager.java + platform-impl/…/LafManagerImpl.kt
**Причина:** проводка фичи — у пары «день/ночь» отдавалась только половина. Платформа хранит обе половины (`preferredLightThemeId`, `preferredDarkThemeId`), но наружу отдаёт лишь тёмную: `getPreferredDarkThemeId()` есть, светлого близнеца нет, поле приватное. Наша страница «Оформление» из-за этого показывала в поле «Днём» не заданную тему, а первую светлую из списка — то есть догадку, выглядящую как факт, и «ОК» эту догадку записывал.
**Изменено:** добавлен `getPreferredLightThemeId()` — `@ApiStatus.Internal`, с телом `return null` в абстрактном классе и `return preferredLightThemeId` в реализации, слово в слово по образцу тёмного близнеца. Поведения платформы не меняет: новый метод никем в апстриме не зовётся.
**Кандидат на отправку в апстрим:** да — это не продуктовая правка, а недостающая половина симметричной пары.
...
### Непубличный API: `UiThemeProviderListManager.getThemeListForTargetUI`

**Зачем:** отличить тему, применимую без перезапуска, от классической. Публичного признака нет:
`UIThemeLookAndFeelInfo.isRestartRequired()` — отдельный атрибут регистрации со значением по
умолчанию `false`, и у платформенных `Darcula` и `IntelliJ` он ЛОЖЬ, хотя в `targetUi="classic"`.
Фильтр по флагу (выпущен в 0.6.24) не отсекал ничего.

**Цена отказа:** переключение на классическую тему под новым интерфейсом роняет
`LafManager.updateUI()` на первом же `ToolbarComboButton` и бросает обход окон на середине —
интерфейс остаётся наполовину в прежней теме.

**Риск синка:** метод внутренний; если уедет — `ThemeTargeting.switchableIds()` вернёт пустое
множество, фильтр выключится и страница покажет все темы, как до починки. Отказ мягкий, не падение.
...
**Когда синкаемся (решение владельца №57 от 08.09.2026):** только по **стабильным** релизам своей
линии. EAP, rc и preview в основу форка не ставим; отдельная нужная правка апстрима берётся руками
по требованию владельца, а не сплошным синком. Следующая цель — `idea/2026.3` (наша линия 263 = 2026.3;
`idea/2026.2.2` — предыдущая линия и назад не мержится).

0. `./vibe-plugins/tools/checkUpstreamLag.sh` — он называет **тег нашей линии** (линия берётся из
   `build.txt`: 263 → 2026.3) и печатает готовую точечную команду. Тег ЧУЖОЙ линии брать нельзя:
   на 08.09.2026 последний стабильный релиз апстрима — `idea/2026.2.2`, а наша база стоит на 263,
   и слияние такого тега откатило бы платформу назад. Пока у нашей линии есть только EAP, синк по
   нему — решение владельца: EAP не релиз.
1. `git fetch upstream tag <тег> --no-tags` — точечно. **Не** `--tags`: их около 2800, и они
   тянут релизные ветки целиком.
