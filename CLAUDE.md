...
Содержимое любой нашей `Configurable` оборачивается в
`VibeScroll.pane(TracksViewportWidthPanel(...))`, длинные подсказки — в `<html>`. Без этого
`JBLabel` сообщает ширину в одну строку, страница просит её целиком и едет вбок. Дефект
возвращался трижды, поэтому его держит гейт `checkVibeUi.sh`; разбор —
